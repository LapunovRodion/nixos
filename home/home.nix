...
# то есть всё, чем ноут отличается от десктопа.
{
  ...
  # =============================================================
  # RustDesk — привязка клиента к своему серверу
  #
  # ID/relay-сервер (hbbs + hbbr) поднят на домашнем сервере, в
  # /opt/rustdesk, и публикует порты ТОЛЬКО на Tailscale-IP: снаружи
  # tailnet его нет вовсе. Клиенту нужны три опции — адрес rendezvous,
  # адрес relay и публичный ключ сервера. Ключ обязателен: hbbs запущен
  # с флагом `-k _`, то есть клиента без ключа он не примет.
  #
  # Почему не xdg.configFile, как у niri: RustDesk хранит в этом же
  # файле свои обычные настройки (тема, вид списка, размеры окна) и
  # переписывает его целиком — в симлинк на /nix/store он писать не
  # сможет. Поэтому файл остаётся обычным, а активация лишь проставляет
  # в нём наши три ключа и не трогает остальное (см. rustdesk-config.py).
  #
  # Ключ здесь ПУБЛИЧНЫЙ (data/id_ed25519.pub с сервера) — секретом он
  # не является, шифровать через agenix нечего.
  #
  # Сам пакет ставится системно — modules/common.nix, блок «Удалённый доступ».
  # =============================================================
  home.activation.rustdeskServer = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    run ${pkgs.python3}/bin/python3 ${./rustdesk-config.py} \
