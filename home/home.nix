  ...
  home.file = lib.mkMerge [
    {
      ...
      # =============================================================
      # Скилл archify для Claude Code (вход archify, см. flake.nix).
      #
      # Источник — ПОДКАТАЛОГ archify/ репозитория, а не его корень: в
      # корне лежат ещё сайт, бенчмарки и эксперименты, а скиллом
      # является каталог, в котором лежит SKILL.md. Имя каталога
      # назначения должно совпадать с полем name: из его frontmatter,
      # иначе Claude Code скилл не подхватит.
      #
      # Каталог получается симлинком в /nix/store, то есть доступен
      # только на чтение. Скилл это переживает: временные файлы он
      # создаёт рядом с ВЫХОДНЫМ html, а кэш проверки обновлений — в
      # ~/.cache/archify-skill. В свой каталог он не пишет ничего.
      #
      # Для работы нужен node в PATH (см. nodejs_22 в common.nix):
      # SKILL.md велит агенту звать `node bin/archify.mjs`.
      #
      # Рядом, в том же ~/.claude/skills, лежит graphify, поставленный
      # руками. Он не мешает: сканируются все подкаталоги, а home-manager
      # владеет только своим.
      # =============================================================
      ".claude/skills/archify".source = "${inputs.archify}/archify";
