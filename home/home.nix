...
in
{
  imports = [
    ...
    ./umbriel.nix         # конфиг композитора — выходы, рамки, правила, бинды
  ];
  ...
  # приложение такой хоткей не узнаёт. Свои бинды композитор переживает
  # (umbriel, как и niri до него, подставляет латинский кейсим уровня 0 как
  # запасной), а вот GTK/Qt/Electron, префикс tmux
  ...
  # custom:types (см. input.keyboard.options в umbriel.nix).
  # =============================================================
  ...
  # Тема курсоров. До этого в системе не было НИ ОДНОЙ — композитор ругался
  ...
  # Имя темы дублируется в input.cursor.theme (см. umbriel.nix).
  ...
  programs.starship = {
    ...
    settings = {
      ...
      # с рамкой активного окна у композитора и с активной вкладкой kitty.
      fill = {
        symbol = "·";
        style = "blue";
      };
  ...
  programs.kitty = {
    ...
    settings = {
      ...
      # Прозрачность. background_blur здесь по-прежнему не нужен: размытие
      # под окном теперь есть, но его даёт композитор (window_rule blur в
      # umbriel.nix), а не kitty.
      # 1.0 — прозрачность целиком отдана композитору (window_rule по
      # app_id kitty), иначе два коэффициента перемножаются. Так фон гаснет
  ...
  # экран на панели по-хорошему незачем (этим занят композитор), но раз уж
  ...
  programs.tmux = {
    ...
    extraConfig = ''
      ...
      # bg=default — фон терминала, то есть прозрачность от композитора сохраняется.
