...
# интерфейс плюс модули на JS. Zen, который стоит рядом (см.
# modules/common.nix), апстрим Natsumi держит в списке НЕсовместимых,
# а Floorp у него помечен рекомендованным — поэтому под мод заведён
# отдельный браузер, а не переоформлен существующий. Zen никуда не
# девается: каталоги профилей разные (~/.config/zen и ~/.floorp),
# пересечься им негде.
#
...
let
  # Имя каталога профиля. В отличие от Zen, где префикс каталога случайный
  # и на каждой машине свой (отсюда опция local.zenProfileDir), профиль
  # заводит home-manager и имя задаём мы — путь одинаков на всех машинах.
  profile = "default";
  ...
  programs.floorp = {
    ...
    profiles.${profile} = {
      ...
      settings = {
        ...
        # Моноширинный на страницах — тот же, что в Zen. Раздельно для
        # латиницы и кириллицы: Firefox держит настройку на каждую систему
        ...
        # Стартовая страница — общий для всех устройств сервис на сервере,
        # см. блок Zen в home/home.nix.
        "browser.startup.homepage" = "https://server.taila27ec6.ts.net:8445";
      };
    };
  };
