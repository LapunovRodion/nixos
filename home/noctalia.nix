...

{
  programs.noctalia = {
    ...
    settings = {
      ...
      # ВСЁ, что под ней.
      #
      # ВЫКЛЮЧЕН намеренно, и с переездом на umbriel причина сменилась.
      #
      # Было: обои уезжали в backdrop niri, фон воркспейса делался
      # прозрачным, и этот слой начинал размывать обои ПОСТОЯННО, а не
      # только под открытой панелью.
      #
      # Стало: размытие под стеклянными панелями (panel.transparency_mode
      # = "glass" выше) даёт сам композитор — layer_rule по неймспейсам
      # noctalia в home/umbriel.nix. Оно и точнее (работает по краям
      # панели, а не по всему экрану), и дешевле. Своему слою backdrop
      # тут делать нечего: он лёг бы вторым размытием поверх первого.
      backdrop = {
        enabled = false;
      ...
      theme = {
        ...
        templates = {
          # "umbriel" из встроенных НЕ включён намеренно: его apply.sh
          # дописывает строку include в ~/.config/umbriel/config.toml, а тот
          # у нас read-only симлинк в /nix/store (см. home/umbriel.nix) —
          # post_hook падал бы на каждой смене обоев. Ровно та же причина,
          # по которой ниже отключён шаблон neovim. Вместо встроенного —
          # свой, templates.user.umbriel; строка include уже стоит в самом
          # сгенерированном конфиге, дописывать её некому и не нужно.
          ...
          # Свой шаблон umbriel. input_path абсолютный (путь в /nix/store),
          ...
          # post_hook не нужен: строка include уже стоит в config.toml,
          # а umbriel сам перечитывает конфиг и включённые файлы при их
          # изменении.
          user.umbriel = {
            input_path = "${./umbriel/theme.toml.in}";
