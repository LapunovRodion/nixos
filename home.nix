{ inputs, lib, pkgs, ... }:
{
  ...
  home.file.".config/zen/vkvdhp86.Default Profile/user.js".text = ''
    ...
    // В профиле задан свой SOCKS-прокси (network.proxy.type=1,
    // 127.0.0.1:10808). Переменные окружения HTTP_PROXY Firefox при этом
    // игнорирует — правит только своя настройка. Прокси в tailnet не ходит,
    // а socks_remote_dns по умолчанию включён, поэтому туда же уходит и
    // резолв имён: *.ts.net превращается в "server not found".
    // Ломало стартовую страницу выше и сохранение записей в Bitwarden —
    // расширение живёт внутри браузера и ходит через тот же прокси.
    user_pref("network.proxy.no_proxies_on", "localhost, 127.0.0.1, ::1, .ts.net, 100.64.0.0/10");

    // На будущее: сейчас DoH не активен (в регионе BY Mozilla его не
    // включает), но если включится — имена MagicDNS сломаются снова,
    // уже по другой причине. Строка ниже страхует заранее.
