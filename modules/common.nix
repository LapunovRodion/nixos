...
# =============================================================
{
  ...
  environment.systemPackages = with pkgs; [
    ...
    tmux
    # smug — сессии tmux из ОПИСАНИЯ, а не из слепка. «Сессия nixos = окна
    #   editor/git/shell, каждое в своём каталоге и со своей командой» лежит
    #   yaml-файлом в этом же репозитории (home/smug/), поэтому одинаково
    #   поднимается и на десктопе, и на ноуте: git pull → ns → smug start nixos.
    #   Этим и отличается от плагина tmux-resurrect: тот снимает ФАКТИЧЕСКОЕ
    #   состояние в ~/.local/share, с абсолютными путями и перезаписью раз в
    #   N минут — мимо git, мимо nix и только для своей машины.
    #   Взят вместо tmuxp и tmuxinator из-за замыкания: 41 МиБ против 213 у
    #   tmuxp (тянет питон) и 94 у tmuxinator (руби). Здесь один Go-бинарь.
    #   Плата за лёгкость: `smug print` снимает живую сессию грубо — теряет
    #   каталоги отдельных панелей и не видит запущенных программ, — так что
    #   конфиги пишутся руками, а print годится лишь на черновик.
    #   Здесь только пакет; сами сессии — xdg.configFile в home/home.nix.
    smug
    ...
    # home/home.nix, здесь только пакет, схема kitty/tmux/smug.
