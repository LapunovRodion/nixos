...
in
{
  ...
  environment.systemPackages = with pkgs; [
    ...
    # ---- Картинки ----
    # Просмотрщик и редактор в одном: кроп, поворот, ресайз, коррекция, плюс
    # пакетные операции над папкой. Закрывает то, чего не делает satty — тот
    # аннотирует только свежий снимок из пайплайна, а не готовый файл с диска.
    # Заодно это единственный в системе хендлер image/* для xdg-open.
    gthumb
    # CLI-утилиты
    gh          # github-cli
    lazygit
